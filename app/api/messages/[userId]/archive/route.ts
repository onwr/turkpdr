...
import { archiveConversation } from "@/lib/queries/messages";
...
export async function PATCH(_request: Request, context: RouteContext) {
  ...
  try {
    await archiveConversation(auth.user.id, partnerId);
    return NextResponse.json({ message: "Konuşma arşivlendi." });
  } catch {
    return NextResponse.json(
      { error: "Konuşma arşivlenemedi." },
      { status: 500 }
    );
  }
}
